...
### 5.21 Full Table List (20 — table #20 added pre-Phase 7, ADR-012)

```
users, permission_grants, user_permissions, groups, group_members,
...
### 5.22 `user_permissions` (table #20 — ADR-012)
...
| permission_name | varchar(255) | No | |
| group_id | bigint unsigned | Yes | FK → groups.id, NULL = global |
| granted_via_grant_id | bigint unsigned | No | FK → permission_grants.id, lineage |
| created_at / updated_at | timestamp | — | |

**Indexes:** unique(user_id, permission_name, group_id); index(user_id)
**Invariants:** read model only — CURRENT effective grants; `permission_grants` remains the append-only ledger and source of truth for history (RULE-005). Maintained atomically with the ledger row in the same transaction.
...
| **ADR-011** | **Explicit `restrictOnDelete()` on `courses.instructor_id`, `permission_grants.granter_id` and `permission_grants.grantee_id` (previously implicit RESTRICT). Users owning courses or grant history cannot be hard-deleted — soft-delete is the only supported removal path once such rows exist. SQLite skips the ALTER (engine already enforces equivalent NO ACTION; SQLite cannot ALTER foreign keys) — MySQL enforces the explicit constraint.** |
| **ADR-012** | **Materialized `user_permissions` read model (table #20) resolving the ledger-scan authorization risk: `permission_grants` stays the append-only audit ledger and source of truth for history (RULE-005); `user_permissions` mirrors CURRENT effective grants only (unique `user_id`,`permission_name`,`group_id`, lineage via `granted_via_grant_id`). Tradeoff accepted: one extra write per grant/revoke, in exchange for O(1) authorization reads. Writes are atomic with the ledger row in the same transaction (service wiring lands in Phase 5; `PermissionReadModelSync` seam created now and tested in isolation).** |
| **ADR-013** | **`manager_depth` (unsigned smallint, default 0, root Admin = 0) added to `users` as a CACHE, not a source of truth — `manager_id` traversal remains authoritative for cycle prevention (ADR-007); depth only accelerates ceiling-rule reads. Maintained on write by recomputing the entire subtree below the reassigned node in the same transaction (`ManagerDepth` seam created now; service wiring lands in Phase 5). No max-depth cap adopted — depth stays advisory-only, open item, not a silent rejection rule.** |
| **ADR-014** | **`activity_log` retention window decided: 90 days (`ACTIVITY_LOG_RETENTION_DAYS`, env-overridable, never hardcoded). Nightly `activitylog:prune` command (daily schedule) deletes rows older than the window, using the new `created_at` index so the run never full-scans. `permission_grants` is explicitly excluded from pruning — permanent delegation ledger (RULE-005) vs. supplementary trail.** |
| **ADR-015** | **Orphan protection for `component_completions` as the operational counterpart to ADR-010 (which justified the missing FK): `ComponentDeletionObserver` registered on Lesson, Assignment and Quiz deletes matching completion rows. Current reality is hard-delete-only (no SoftDeletes in the codebase), so `deleted()` purges; `forceDeleted()` also purges so a future soft-delete adoption stays correct; a future soft-delete (trashed) event must KEEP completions so restores resurrect progress — recorded here so the behavior is never assumed silently later.** |
